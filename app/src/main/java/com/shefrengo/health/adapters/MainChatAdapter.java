...
import com.shefrengo.health.utils.SetTime;
...
public class MainChatAdapter extends RecyclerView.Adapter<MainChatAdapter.ViewHolder> {
    ...
    private final List<Conversations> chatsList;
    ...
    public void onBindViewHolder(@NonNull @NotNull ViewHolder holder, int position) {
        ...
        String username = usersList.get(position).getName() + " "+usersList.get(position).getSurname();
        String photo = usersList.get(position).getProfilePhotoUrl();
        int messagecount = chatsList.get(position).getUnreadChatCount();

        holder.name.setText(username);
        Glide.with(context).asBitmap().load(photo).placeholder(R.drawable.ic_profile).into(holder.circleImageView);
